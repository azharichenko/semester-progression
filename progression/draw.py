...
def calculate_times(config, *, key=None, struct=None, scaling_factor=1):
    if key:
        return (
            determine_time_length(config['start'], config[key]['start'], scaling_factor),
...
def create_new_image():
    ...
    midterm_start, midterm_end = calculate_times(config, key='midterm', scaling_factor=scaling_factor)
    finals_start, finals_end = calculate_times(config, key='finals', scaling_factor=scaling_factor)
    ...
    for struct in config['breaks']:
        start, end = calculate_times(config, struct=struct, scaling_factor=scaling_factor)
...
def draw_display_message(text: str):
    intuitive_font = ImageFont.truetype(Intuitive, 22)

    img = Image.new("P", size=(InkyPHAT.WIDTH, InkyPHAT.HEIGHT))
    ImageDraw.Draw(img)

    text_w, text_h = intuitive_font.getsize(text)
    text_x = (InkyPHAT.WIDTH - text_w) // 2
    text_y = (InkyPHAT.HEIGHT - text_h // 2)
    img.text((text_x, text_y), text, InkyPHAT.BLACK, font=intuitive_font)
